...
- **Solidity SMTChecker** — `solc` built-in CHC checker ([smtchecker](https://docs.soliditylang.org/en/latest/smtchecker.html)). Assertions and `require`s are proved or refuted on the Solidity AST. There is no second spec language. The baseline is [`verification/smtchecker/`](verification/smtchecker/): solc 0.8.24, the CHC engine, and z3. It proves the deposit and withdraw accounting assertions, including that another address's credit is unchanged. It does not prove the mapping-sum invariant, and it treats `transfer` as an untrusted external call.
