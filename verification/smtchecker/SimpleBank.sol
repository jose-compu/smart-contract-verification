...
/// @dev `totalCredits` is a ghost. It is not in `src/SimpleBank.sol`.
///      `specDepositPreservesOther` and `specWithdrawPreservesOther` are witness
///      transactions for P3. They apply `_credit` / `_debit`. The CHC engine may call them.
contract SimpleBank {
    /// @notice Credited ETH balance of each depositor, in wei.
    mapping(address => uint256) public balances;

    /// @notice Net wei credited minus wei debited. Not coupled to `address(this).balance`.
    ...
    function withdraw(uint256 amount) external {
        ...
        assert(totalCredits == sumBefore - amount);
